...
export default class RunCatPreferences extends ExtensionPreferences {
  ...
  async fillPreferencesWindow(window: Adw.PreferencesWindow) {
    ...
    const page = this.#builder.get_object<Adw.PreferencesPage>(
      "preferences-general",
    );
  ...
  #setupPage() {
    ...
    // Displaying Items
    const combo = this.#builder!.get_object<Adw.ComboRow>(
    ...
    combo.set_selected(
      this.#settings!.get_enum(gioSettingsKeys.DISPLAYING_ITEMS),
    );

    combo.connect(
      "notify::selected",
      (/** @type {Adw.ComboRow} */ { selected }: Adw.ComboRow) => {
        this.#settings!.set_enum(gioSettingsKeys.DISPLAYING_ITEMS, selected);
      },
    );

    // Panel Position
    const positionCombo = this.#builder!.get_object<Adw.ComboRow>(
      gioSettingsKeys.PANEL_POSITION,
    );

    positionCombo.set_selected(
      this.#settings!.get_enum(gioSettingsKeys.PANEL_POSITION),
    );

    positionCombo.connect(
      "notify::selected",
      (/** @type {Adw.ComboRow} */ { selected }: Adw.ComboRow) => {
        this.#settings!.set_enum(gioSettingsKeys.PANEL_POSITION, selected);
      },
    );
    ...
    this.#settings!.bind(
      ...
      this.#builder!.get_object<Adw.ExpanderRow>(
        gioSettingsKeys.customSystemMonitor.ENABLED,
      ),
      "enable-expansion",
      Gio.SettingsBindFlags.DEFAULT,
    ...
    this.#settings!.bind(
      ...
      this.#builder!.get_object<Adw.EntryRow>(
        gioSettingsKeys.customSystemMonitor.COMMAND,
      ),
      "text",
      Gio.SettingsBindFlags.DEFAULT,
    ...
    this.#builder!.get_object<Gtk.Button>("reset").connect("clicked", () => {
      ...
      // Panel Position
      this.#settings!.reset(gioSettingsKeys.PANEL_POSITION);
      positionCombo.set_selected(
        this.#settings!.get_enum(gioSettingsKeys.PANEL_POSITION),
      );
    });
  ...
  #setupMenu() {
    ...
    homepageAction.connect("activate", () =>
    ...
    aboutAction.connect("activate", () => {
      const logo = Gtk.Image.new_from_file(
        `${this.path}/resources/se.kolesnikov.runcat.svg`,
      );

      const aboutDialog =
        this.#builder!.get_object<Adw.AboutWindow>("about-dialog");

      aboutDialog.set_property("logo", logo.get_paintable());
      aboutDialog.set_property(
        "version",
        `${_("Version")} ${this.metadata.version}`,
      );
