...
try {
  ...
  console.error(
    `${LOG_PREFIX}: GTop is not installed. Falling back to /proc/stat parsing`,
  );
}
...
async function getCpuStatsFallback(): Promise<CpuStats> {
  ...
  const data = contents
    ...
    .reduce<Record<string, CpuStats>>((acc, line) => {
